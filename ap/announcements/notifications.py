...
def request_statuses(trainee):
  ...
  return [(messages.ERROR, message.format(url=reverse('attendance:attendance-submit') + '?slip_type=leaveslip&id=' + str(req.id), request=req._meta.verbose_name)) if isinstance(req, IndividualSlip) else (messages.ERROR, message.format(url=req.get_absolute_url(), request=req._meta.verbose_name)) for req in requests]
