...
from django.forms.widgets import DateInput, TimeInput, RadioSelect, SelectMultiple, Widget, Select, MultiWidget
...
class SelectTimeWidget(Widget):
  ...
  def render(self, name, value, attrs=None):
    ...
    if hour_val != '': hour_val = u"%.2d" % hour_val
    if minute_val != '': minute_val = u"%.2d" % minute_val
    if second_val != '': second_val = u"%.2d" % second_val
