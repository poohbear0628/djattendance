...
import { ATTENDANCE_STATUS_LOOKUP, SLIP_STATUS_LOOKUP, EVENT_CODE_LOOKUP, joinValidClasses } from '../constants'
...
const EventView = ({ event, roll, slip, onClick, selectedEvents }) => {
  ...
  if (roll || slip) {
    return (
      <OverlayTrigger trigger="hover" placement="bottom" overlay={<Popover style={{display: "inline-block"}}>{rollPopover}<br></br> {slipPopover}</Popover>}>
