...
def initialize_test(testname, drivername="sauce", ciname="travisci", urlname=None, email=None, password=None, ta_email=None, ta_password=None):
  ...
  """
  global auto
  auto = AutomationSetup(testname, drivername, ciname)
  auto.set_webdriver()
  global driver
  driver = auto.get_webdriver()
  ...
  # parsing the URL option
  if urlname:
    auto.set_urladdress(urlname)
  else:
    auto.set_urladdress(login_data["domain"])

  # parsing login option
  if email:
    auto.set_email(email)
  else:
    auto.set_email(login_data["username"])
  if password:
    auto.set_password(password)
  else:
    auto.set_password(login_data["password"])
  if ta_email:
