...
def finalize_test():
	...
	if auto.is_sauce_used():
		...
		else: auto.update_saucelab(True)
	driver.quit()
...
def wait_for(by, value, check_for='presence', time=10):
	...
	"""
	try:
		if by == 'class': method = By.CLASS_NAME
		elif by == 'CSS': method = By.CSS_SELECTOR
		elif by == 'id': method = By.ID
		elif by == 'name': method = By.NAME
		elif by == 'tag': method = By.TAG_NAME
		elif by == 'xpath': method = By.XPATH
		elif by == 'text': method = By.XPATH; value = '//*[contains(text(), "'+value+'")]'
		elif by == 'value': method = By.XPATH; value = '//*[@value="'+value+'"]'
...
def get_the_element(by, value, pose=0):
	...
	"""
	time.sleep(pose)
	if by == "id": return driver.find_element_by_id(value)
	elif by == "name": return driver.find_element_by_name(value)
...
def get_child_element(parent, child, by='css'):
	...
	"""
	if by == "id": return driver.find_element_by_id(parent).find_element_by_id(child)
	elif by == "name": return driver.find_element_by_name(parent).find_element_by_name(child)
	elif by == "xpath": return driver.find_element_by_xpath(parent).find_element_by_xpath(child)
	elif by == "class": return driver.find_element_by_class_name(parent) \
									 .find_element_by_class_name(child)
	else: return driver.find_element_by_css_selector(parent).find_element_by_css_selector(child)
...
def click_element_actionchain(by, value, pose=0):
	...
	"""
	if by == "text": elem = driver.find_element_by_xpath('//*[contains(text(), "'+value+'")]')
	elif by == "value": elem = driver.find_element_by_xpath('//*[@value="'+value+'"]')
	elif by == "id": elem = driver.find_element_by_id(value)
	elif by == "class": elem = driver.find_element_by_class_name(value)
	elif by == "CSS": elem = driver.find_element_by_css_selector(value)
	else: elem = driver.find_element_by_xpath(value)
...
def generate_report(suite, testname, format='html'):
	...
	if format == 'text':
    	...
    	runner = HTMLTestRunner.HTMLTestRunner(
                	stream=fp,
                	title=testname,
                	description='Report output by HTMLTestRunner.'
             		)
...
def search_and_select(by, value, partial_text, text, pose=0):
	...
	try:
		...
		if DEBUG: print "drop-down menu:	", options
...
#4. def get_request_summary():
""" get contents of submitted request (eg. leaveslip) """

#5. def get_report_contents():
""" opens in another window, get the contents and compare with the file """
""" PDF report form, HTML form ? """

#6. def element_drag_and_drop():

#7. def element_hovoer_over():
""" condition check if popup message needs to return """

#8. def open_tab_execute_operations():
	# use eval()
	# def func2(expression):
	# ...   eval(str(expression))

#9. def refresh_current_window():

#10. def select_date_on_popup_calendar():

#11. def switch_to_window():
